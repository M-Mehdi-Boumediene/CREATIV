...
function headerSize($scroll){
    ...
    if(!$j('header.page_header').hasClass('centered_logo')){
        ...
        $j('.q_logo a img').css('height','auto');
    }else{
...
/*
**	Initialize MF Slider
*/
var default_header_style;
function initMFSlider(){
    "use strict";
    ...
    if($j('.carousel').length){
        $j('.carousel').each(function(){
            ...
            if($this.hasClass('full_screen')){
                ...
                $this.find('.mf_slider_preloader').css({'height': ($j(window).height() - mobile_header) + 'px'});
                ...
                mobile_header = $j(window).width() < 1000 ? $j('header.page_header').height() - 6 : 0; // 6 is because of the display: inline-block
                $this.find('.mf_slider_preloader').css({'height': ($this.height() - mobile_header) + 'px', 'display': 'block'});
            }
                ...
                if($this.find('.item:first-child .mobile-video-image').length > 0){
                    ...
                    if (src) {
                        ...
                        $j(backImg).load(function(){
                            $j('.mf_slider_preloader').fadeOut(500);
                            initSlider();
                        });
                    }
                }
...
** Init MF Carousel
*/
function initMFCarousel(){
    "use strict";

    if($j('.mf_carousels').length){
        $j('.mf_carousels').flexslider({
            animationLoop: true,
            controlNav: false,
            useCSS: false,
            pauseOnAction: true,
...
function countClientsPerRow(){
    ...
    if($j('.mf_clients').length){

        $j('.mf_clients').each(function() {
            var $clients = $j(this);
            var mf_clients_height = $clients.height();
            var mf_clients_width = $clients.width();
            var maxHeightClient;
            var clientWidth = $clients.find('.mf_client_holder').width();
            var countClient = $clients.find('.mf_client_holder').length;
            $clients.find('.mf_client_holder').each(function() {
                maxHeightClient = maxHeightClient > $j(this).height() ? maxHeightClient : $j(this).height();
            });
            maxHeightClient = maxHeightClient + 35; //margin for client is 35
            var numberOfRows  = Math.ceil(mf_clients_height / maxHeightClient);
            var numberOfClientsPerRow =  Math.ceil(mf_clients_width/clientWidth);
            var numberOffullRows = Math.floor(countClient / numberOfClientsPerRow);
            var numberOfClientsInLastRow = countClient - (numberOfClientsPerRow * numberOffullRows);
            if(numberOfClientsInLastRow === 0){
                numberOfClientsInLastRow = numberOfClientsPerRow;
            }
            $clients.find( ".mf_client_holder" ).removeClass('border-bottom-none');
            var item_start_from = countClient - numberOfClientsInLastRow - 1;
            $clients.find( ".mf_client_holder:gt("+ item_start_from +")" ).addClass('border-bottom-none');
        });
    }
}
...
function countAnimatedTextIconPerRow(){
    ...
    if($j('.animated_icons_with_text').length){
        $j('.animated_icons_with_text').each(function() {
            var $icons = $j(this);
            var mf_icons_height = $icons.height();
            var mf_icons_width = $icons.width();
            var maxHeightIcons;
            ...
            var numberOfIconsPerRow =  Math.ceil((mf_icons_width/iconWidth));
...
function initSearchButton(){
    if($j('.search_button').length){
        $j('.search_button').click(function(e){
            ...
            if($j('html').hasClass('touch')){
                if ($j('.mf_search_form').height() == "0") {
                    $j('.mf_search_form input[type="text"]').onfocus = function () {
                        window.scrollTo(0, 0);
                        document.body.scrollTop = 0;
                    };
                    $j('.mf_search_form input[type="text"]').onclick = function () {
                        window.scrollTo(0, 0);
                        document.body.scrollTop = 0;
                    };
                    $j('.header_top_bottom_holder').css('top','50px');
                    $j('.mf_search_form').css('height','50px');
                    $j('.content_inner').css('margin-top','50px');
                    if($scroll < 34){ $j('header.page_header').css('top','0'); }
                } else {
                    $j('.mf_search_form').css('height','0');
                ...
                $j(window).scroll(function() {
                    if ($j('.mf_search_form').height() != "0" && $scroll > 50) {
                        $j('.mf_search_form').css('height','0');
                        $j('.header_top_bottom_holder').css('top','0');
                        $j('.content_inner').css('margin-top','0');
                    }
                ...
                $j('.mf_search_close').click(function(e){
                    e.preventDefault();
                    $j('.mf_search_form').css('height','0');
                    $j('.header_top_bottom_holder').css('top','0');
                    $j('.content_inner').css('margin-top','0');
                ...
                if ($j('.mf_search_form').height() == "0") {
                    $j('.mf_search_form input[type="text"]').focus();
                    $j('.header_top_bottom_holder').stop().animate({top:"50px"},150);
                    $j('.mf_search_form').stop().animate({height:"50px"},150);
                    $j('.content_inner').stop().animate({marginTop:"50px"},150);
                    if($scroll < 34){ $j('header.page_header').stop().animate({top:0},150); }
                    $j('.title.has_fixed_background').css('backgroundPosition', 'center '+(yPos+50)+'px');
                } else {
                    $j('.mf_search_form').stop().animate({height:"0"},150);
